...
#include "../inc/Siec.hh"
#define ROZMIAR 40   // maks liczba powtorzonych nazw przystankow
...
Przystanek *Siec::ZnajdzPrzystanek(std::string Arg) const {
    ...
    while(i<this->IloscPrzystankow && !CzyZnaleziony){
        if(this->SpisPrzystankow[i]->getNazwa()==Arg) CzyZnaleziony=true;
        else ++i;
    }
    if(CzyZnaleziony)return this->SpisPrzystankow[i];
